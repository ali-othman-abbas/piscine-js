...
function buildRanges() {
  ...
  widthSlider.addEventListener('input', (_) => {
    allGossips.forEach(ele => ele.style.width = `${widthSlider.value}px`)
  })
  fontSizeSlider.addEventListener('input', (_) => {
    allGossips.forEach(ele => ele.style.fontSize = `${fontSizeSlider.value}px`)
  })
  backgroundSlider.addEventListener('input', (_) => {
    allGossips.forEach(ele => ele.style.backgroundColor = `hsl(280, 50%, ${backgroundSlider.value}%)`)
  })
